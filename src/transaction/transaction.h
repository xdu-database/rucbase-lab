...
class Transaction {
   ...
   private:
    ...
    /** 用于索引任务: The pages that were latched during index operation, used for concurrent index */
    std::shared_ptr<std::deque<Page *>> page_set_;
    /** 用于索引任务: Concurrent index: the page IDs that were deleted during index operation.*/
